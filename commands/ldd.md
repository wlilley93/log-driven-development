...
Hold the spine and the disciplines:
- **Twin ledgers:** harvest intent into `_harvest/*` (provenance or it doesn't go in); journal every decision +
  its reason into `metacognition/*` (+ an `INDEX.md` pointer). The orchestrator is the **only** writer of shared
  state (one-writer rule) - spawned agents return their what/why.
...
If the argument is `status`, instead report where the loop is: the verified envelope, open gaps from the last
sweep, and the single next move.
