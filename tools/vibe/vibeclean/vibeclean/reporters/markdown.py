...
from vibeclean.models import Finding, ScanResult, Severity, RunnerStatus
...
class MarkdownReporter(BaseReporter):
    """Concise markdown report for PR comments."""
    ...
    def render_to_string(self) -> str:
        ...
        self._write_category_table(lines)
    ...
    def _write_header(self, lines: list[str]) -> None:
        ...
        lines.append(f"## {icon} vibeclean: {status}")
        lines.append("")
        lines.append(
    ...
    def _write_category_table(self, lines: list[str]) -> None:
    ...
    def _write_top_findings(self, lines: list[str]) -> None:
        ...
        for f in findings:
            ...
            if len(msg) > 120:
                msg = msg[:117] + "..."
            lines.append(
                f"| {emoji} {f.severity.value} | {f.category.value} | {loc} | {msg} |"
            )
    ...
    def _write_footer(self, lines: list[str]) -> None:
        lines.append("---")
        lines.append(
            "*Generated by [vibeclean](https://github.com/vibeclean/vibeclean)*"
        )
        lines.append("")
